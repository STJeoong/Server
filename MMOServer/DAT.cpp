...
bool DAT::move(int id, const AABB& newAABB)
{
    this->removeLeaf(id);
    _nodes[id].aabb = newAABB;
    this->insertLeaf(id);
...
int DAT::findBestSibling(int leaf)
{
    ...
    while (!_nodes[idx].isLeaf()) // node != leaf 면 left, right모두 -1이 아님. 반대로 node == leaf면 둘다 -1임.
    {
        ...
        int area = _nodes[idx].aabb.perimeter();
        AABB combinedAABB = _nodes[idx].aabb + _nodes[leaf].aabb;
        int combinedArea = combinedAABB.perimeter();
        int cost = 2 * combinedArea; // 새로운 group AABB를 만드는 비용 ( 현재 노드가 나타내는 그룹과 새로운 리프노드가 합쳐져서 새로운 그룹을 생성 ). 왜 2를 곱하는 진 모르겠음.
        int inheritanceCost = 2 * (combinedArea - area); // 기존의 group AABB를 확장하는 비용 ( 왼쪽 or 오른쪽 자식 그룹에 리프노드 추가 )

        int leftCost = inheritanceCost + this->cost(left, _nodes[leaf].aabb); // 왼쪽 자식 그룹에 추가할 비용
        int rightCost = inheritanceCost + this->cost(right, _nodes[leaf].aabb); // 오른쪽 자식 그룹에 추가할 비용
...
int DAT::cost(int i, const AABB& aabb) const
{
    AABB combinedAABB = _nodes[i].aabb + aabb;
    int combinedArea = combinedAABB.perimeter();

    int cost = combinedArea;
    if (!_nodes[i].isLeaf())
    {
        int oldArea = _nodes[i].aabb.perimeter();
