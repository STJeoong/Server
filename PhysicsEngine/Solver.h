...
public:
	...
	//void solvePositionConstraints(const std::vector<Collision2D*>& collisions, float dt);
private:
	void impulse(const Collision2D& c, const std::vector<float>& jaco, const std::tuple<float, float, float, float>& mass, float lambda);
	float computePenetrationBias(const Collision2D& c, float dt);
	void computeBouncinessBias(Collision2D& c);
	...
	std::vector<float> _normalEffMasses;
	std::vector<float> _tangentEffMasses;
	std::vector<std::vector<float>> _normalJacobians;
	std::vector<std::vector<float>> _tangentJacobians;
